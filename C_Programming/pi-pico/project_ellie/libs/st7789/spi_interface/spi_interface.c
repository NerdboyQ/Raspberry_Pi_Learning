...
display_err_t display_init(bool spi0_idx, int8_t reset_pin, int8_t data_cmd_pin, int8_t backlight_pin, st7789_handle_t *g_st7789_handle) {
    ...
    if (spi0_idx) {
        ...
        display.spi_pin_clk = SPI_PIN_SCK;
        display.spi_pin_cs = SPI_PIN_CS;
        display.spi_pin_mosi = SPI_PIN_MOSI;
        display.spi_pin_miso = SPI_PIN_MISO;
    } else {
        spi_init(spi1, SPI_BAUD_RATE);
        display.spi_pin_clk = SPI_PIN_SCK+8;
        display.spi_pin_cs = SPI_PIN_CS+8;
        display.spi_pin_mosi = SPI_PIN_MOSI+8;
        display.spi_pin_miso = SPI_PIN_MISO+8;
    }
